...
public class PHobbyRestaurant {
    ...
    private static boolean jsonMenuLacksCategory(JsonArray foods) {        
                    ...
                    while (dessertsIter.hasNext()) {
                        ...
                        if (totalCost <= budget) {
                            ...
                            if (thisSatisfaction > highestSatisfaction) {
                                mostSatisfactoryMeal = new Meal(thisApp, thisDrink, thisMC, thisDessert, totalCost, thisSatisfaction);
                                highestSatisfaction = thisSatisfaction;
                            }
                            mostSatisfactoryMeal = new Meal(thisApp, thisDrink, thisMC, thisDessert);
                            highestSatisfaction = mostSatisfactoryMeal.getSatisfaction();                            
                        }
                    }
                }
            }
    ...
    private static void printMostSatisfactoryMeal(Meal mostSatisfactoryMeal, int budget) {
        ...
        if (mostSatisfactoryMeal.getSatisfaction() >= 0) { // This found a meal
            System.out.print("The meal from which you will derive the most satisfaction is the following: \n" + 
                             ...
                             ".00, which is well within your budget of $" + budget +  ".00. (Leave your server a nice tip of $" + (budget - mostSatisfactoryMeal.getCost() ) + ".00.) \n");
        } 
        else { //No meal found.
